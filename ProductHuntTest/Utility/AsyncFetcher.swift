...
class AsyncFetcher {
    ...
    private var completionHandlers = [UUID: [(Data?) -> Void]]()

    /// An `NSCache` used to store fetched objects.
    private var cache = NSCache<NSUUID, NSData>()
    ...
    */
    func fetchAsync(_ identifier: UUID, imageURL: String, completion: ((Data?) -> Void)? = nil) {
     ...
     */
    func fetchedData(for identifier: UUID) -> NSData? {
     ...
     */
    private func fetchData(for identifier: UUID, imageURL: String) {
        ...
        if let data = fetchedData(for: identifier) {
            ...
            invokeCompletionHandlers(for: identifier, with: data as Data)
        } else {
            // Enqueue a request for the object.
            let operation = AsyncFetcherOperation(identifier: identifier, imageURL: imageURL)
            ...
            operation.completionBlock = { [weak operation] in
                ...
                self.cache.setObject(fetchedData as NSData, forKey: identifier as NSUUID)
     ...
     */
    private func invokeCompletionHandlers(for identifier: UUID, with fetchedData: Data) {
